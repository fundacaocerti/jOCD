...
public class FlashBuilder {
    ...
    public void addData(long address, byte[] data) {
        ...
        if (address < this.flashStart) {
            Log.e(TAG, "Invalid flash address " + String.format("%08X", address) + " is before flash start " + String.format("%08X", this,flashStart));
            return;
        }
        ...
        for (FlashOperation op : flashOperations) {
            if (prevOp != null) {
                if ((prevOp.address + prevOp.data.length) > op.address) {
                    Log.e(TAG, "Error adding data - Data at " +
                            String.format("%08X", prevOp.address) + ".." +
                            String.format("%08X", prevOp.address + prevOp.data.length) +
                            " overlaps with " +
                            String.format("%08X", op.address) + ".." +
                            String.format("%08X", op.address + op.data.length));
                    return;
                }
            }
            prevOp = op;
     ...
     */
    // TODO
    public ProgrammingInfo program(boolean chipErase, ProgressUpdateInterface progressUpdate, boolean smartFlash, boolean fastVerify) {
        ...
        for (FlashOperation op : flashOperations) {
            ...
            while (pos < op.data.length) {
                ...
                if (flashAddress != pageDataEnd) {
                    byte[] oldData = this.flash.target.readBlockMemoryUnaligned8(pageDataEnd,
                            flashAddress - pageDataEnd);
